// ─────────────────────────────────────────────────────────────────────────────
// 선박 안전점검 마스터 데이터 (정적 파일)
//
// Prisma/Firebase DB에 저장하지 않고 앱 번들에 포함.
// • 22개 섹션, 325개 항목이 절대 변경되지 않으므로 DB 조회 불필요
// • 각 항목의 ID는 itemNo 기반 결정적(deterministic) 값 사용
//   예: itemNo "4.8" → id "4_8",  "10.22" → "10_22"
// ─────────────────────────────────────────────────────────────────────────────

export type MasterItem = {
  id: string;          // itemNo 기반 안정적 ID (예: "1_1", "10_22")
  itemNo: string;      // 항목 번호 (예: "1.1", "10.22")
  description: string;
  riskScore: number;
  order: number;       // 전체 정렬 인덱스 (1–325, Excel/PDF 순서 보장)
};

export type MasterSection = {
  id: string;          // 안정적 ID (예: "sec_1", "sec_22")
  no: number;
  nameEn: string;
  order: number;
  items: MasterItem[];
...
// ── 항목 목록 [itemNo, sectionNo, description, riskScore] ─────────────────────
